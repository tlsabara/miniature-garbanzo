...
@pytest.fixture
def fxt_new_perm_data(
    desc_gperms=None, sys_name_gperms=None, long_desc_gperms=None, app_gperms=None
) -> dict:
    """
    ...
    long_desc_gperms = (
        long_desc_gperms if long_desc_gperms else "Descrição completa da permissaor"
    )
...
def fxt_new_link_type(value=None) -> dict:
    ...
    return {"desc_garbazno_link": value}
...
def fxt_new_asset_class(accepted_links=None, desc_asset_class=None) -> dict:
    ...
    return {"desc_asset_class": desc_asset_class, "accepted_links": accepted_links}
...
@pytest.fixture
def fxt_new_extra_field_type(
    name=None, description=None, model=None, fixed_values=None
) -> dict:
    """
...
def fxt_new_extra_field(item=None, field_type=None, value=None) -> dict:
    ...
    return {"item": item, "field_type": field_type, "value": value}
...
@pytest.mark.parametrize(
    "value", ["asset_to_user", "asset_to_asset", "asset_to_location"]
)
...
def fxt_create_garbanzoassetclass(
    fxt_create_garbanzolinktype, fxt_new_asset_class
) -> object:
    ...
    accepted_link_items = GarbanzoLinkType.objects.filter(
        desc_garbazno_link__in=fxt_new_asset_class["accepted_links"]
    )
...
@pytest.mark.parametrize(
    ...
    [("Computador", "Asset Device"), ("Webcam", "Asset Accessory")],
)
def fxt_create_garbanzoassettype(
