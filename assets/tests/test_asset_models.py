...
def test_garbanzoassetclass_model(fxt_create_garbanzolinktype, fxt_new_asset_class):
    ...
    accepted_link_items = GarbanzoLinkType.objects.filter(desc_garbazno_link=fxt_create_garbanzolinktype)
...
@pytest.mark.django_db
def test_garbanzoassetitem_model_without_custom_fields(fxt_create_garbanzoassettype, fxt_new_asset_item) -> None:
    ...
    assert str(item) == f"({fxt_new_asset_item['type_asset']}) - {fxt_new_asset_item['name_item']} - ASSET:{fxt_new_asset_item['asset_number']}"
